...
export default function RegisterPage() {
  ...
  async function handleRegister(e) {
    ...
    const {
      data: teamAlreadyExists,
      error: teamCheckError,
    } = await supabase.rpc("team_name_exists", {
      check_team_name: teamName.trim(),
    });

    if (teamCheckError) {
      console.error(
        "Team name availability check failed:",
        teamCheckError
      );

      setMessage(
        "We could not check the Predictor Team Name. Please try again."
      );
    ...
    if (teamAlreadyExists) {
      ...
      setMessage(
        `"${teamName.trim()}" is already being used. Please choose a different Predictor Team Name.`
      );
    ...
    if (error) {
      /*
        ...
        protection in case two people choose the same
        name at almost exactly the same time.
      */
  ...
  return (
    <main className="wow-register-page">
                ...
                <label>
                  ...
                  >
                    {teamNameError
                      ...
                      : "Team names must be unique — choose a name that nobody else is using."}
                  </div>
                </label>
      ...
      <style jsx>{`
        ...
        .team-name-help.error {
          color: #ff747a;

          font-weight: 900;
        }

        .team-name-error input {
          border-color:
            #ed1c24 !important;
